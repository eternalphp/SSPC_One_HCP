...
{
    public class HcpDataInfoService : IHcpDataInfoService
    {
        ...
        public HcpDataInfoService(IEfRepository rep, ICommonService commonService, ISystemService systemService)
        {
            _rep = rep;
        ...
        /// <returns></returns>
        public ReturnValueModel GetHcpDataInfoList(RowNumModel<DataInfoSearchViewModel> rowNum, WorkUser workUser)
        {
            var isAdmin = _commonService.IsAdmin(workUser);

            ReturnValueModel rvm = new ReturnValueModel();
            var list = (from a in _rep.Where<HcpDataInfo>(s => s.IsDeleted != 1 && s.MediaType == 1
                        && (s.IsCompleted == null
            ...
            var rowlist = rows.ToList().Select(x => new HcpDataInfoOutDto
            {
                Id = x.Id,
                ...
                ProIds = _rep.Where<HcpMediaDataRel>(y => y.DataInfoId == x.Id && y.IsDeleted == 0).Select(y => y.ProId).Distinct().ToList(),
