...
{
    public class WxOAuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            var user = await rep.FirstOrDefaultAsync<WxUserModel>(s => s.IsDeleted != 1 && s.Id == context.UserName );

            // var user = await ContainerManager.Resolve<IAuthRepository>().FindWxUser(context.UserName, "");
            ...
            wxRegisterService.CacheWxUser(user);
