...
namespace SSPC_One_HCP.KBS.Webs.Clients
{
    /// <summary>
    /// Http请求
    /// </summary>
    public abstract class HttpRequestBase<TRequest> where TRequest : IRequest<TRequest>
    {
        ...
        {
            _headers.Add(key, value.ToString().Trim());
            return This();
        }
        ...
        {
            var contentType = _contentType.ToString().Trim().ToLower();
            switch (contentType)
            {
                case "application/x-www-form-urlencoded":
                    return new FormUrlEncodedContent(_params.ToDictionary(t => t.Key, t => t.Value.ToString().Trim()));
